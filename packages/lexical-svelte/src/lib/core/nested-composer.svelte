<script lang="ts" module>
	import warnOnlyOnce from '$lib/shared/warn-only-once.js';
	import type {
		LexicalEditor,
		EditorThemeClasses,
		Klass,
	...
	import { createLexicalComposerContext, setComposerContext, useLexicalComposerContext } from './composer-context.js';
...
<script lang="ts">
	...
	}: LexicalNestedComposerProps = $props();

	const parentContext = useLexicalComposerContext();

	if (parentContext === null) {
		throw new Error('LexicalNestedComposer must be used within a LexicalComposer');
	}
	...
	$effect(() => {
		if (!skipEditableListener) {
			const editableListener: EditableListener = (editable) => initialEditor.setEditable(editable);
			editableListener(parentEditor.isEditable());
			return parentEditor.registerEditableListener(editableListener);
		}
	});

	setComposerContext([initialEditor, context]);
</script>
